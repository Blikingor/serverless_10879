...
function handleLogs() {
  ...
  if (logs) {
    ...
    const defaultDestinationArn = `arn:${partition}:logs:${region}:${this.accountId}:log-group:${logGroupName}`;
    let resourceArn;

    if (typeof logs.accessLogging === 'object' && logs.accessLogging !== null) {
      // Destination supported behavior, defaulting to log stream
      accessLoggingEnabled = logs.accessLogging.enabled;
      resourceArn = logs.accessLogging.destinationArn
        ? logs.accessLogging.destinationArn
        : defaultDestinationArn;
      logFormat = logs.accessLogging.format;
    } else {
      // Legacy behavior
      accessLoggingEnabled = logs.accessLogging == null ? true : logs.accessLogging;
      resourceArn = defaultDestinationArn;

      if (logs.format) {
        logFormat = logs.format;
      }

      // TODO: Worth printing deprecation message?
    }
